...
impl FromStr for RestfulUri {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // from the start of the string, match if there is three or four alpha
        // characters before "://"
        let expression = regex::Regex::new(r"^[a-z]{3,4}:\/\/").unwrap();
        if let Some(prefix) = expression.captures(s) {
            // safe to unwrap because we know the tring contains a colon based
            // on the regex
            let colon_idx = s.find(':').unwrap();
            let scheme = &(&prefix[0])[..colon_idx];
            let postfix = s.to_string().split_off(colon_idx + 3);
            match UriScheme::from_str(scheme)? {
                UriScheme::Tcp => {
                    let (ip_addr, port) = parse_tcp_input(&postfix)?;
                    Ok(Self::Tcp(ip_addr, port))
                }
                UriScheme::Unix => {
                    if postfix.is_empty() {
                        return Err(anyhow!("unix socket path must be provided"));
                    }
...
mod tests {
    ...
    fn uri_input_parsing() {
        ...
        // unix missing path
        assert_eq!(true, RestfulUri::from_str("unix://").is_err());

        // unix with missing slashes
        assert_eq!(true, RestfulUri::from_str("unix:").is_err());
        ...
        // tcp missing port
        assert_eq!(true, RestfulUri::from_str("tcp://localhost").is_err(),);
